"use client";
// app/page.tsx — Javari Education · CR AudioViz AI · EIN 39-3646201
import { useState } from "react";
const TOOLS = [{"icon": "\ud83d\udccb", "label": "Lesson Plan", "desc": "Standards-aligned lesson plans", "href": "/lesson"}, {"icon": "\ud83d\udcdd", "label": "Assessment", "desc": "Quizzes and rubrics", "href": "/assessment"}, {"icon": "\ud83d\udce7", "label": "Parent Email", "desc": "Progress and update emails", "href": "/email"}, {"icon": "\ud83d\udcda", "label": "Curriculum", "desc": "Unit planning and mapping", "href": "/curriculum"}];
export default function Page() {
  ...
  async function go() { if(!input.trim())return; setLoading(true);setOut("");
    try { const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:input}],stream:false,systemOverride:"You are an expert educator and instructional designer. Create engaging, standards-aligned educational content."})});
  ...
  return (<div style={{minHeight:"100vh",background:"#040912",color:"#e2e8f0",fontFamily:"system-ui"}}>
    <nav style={{background:"#1E3A5F",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>🎓</span><span style={{fontWeight:800,color:"#00B4D8",fontSize:15}}>Javari Education</span></div>
    ...
    <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"56px 24px 48px",textAlign:"center"}}>
      <h1 style={{fontSize:"clamp(24px,4vw,44px)",fontWeight:900,color:"#fff",margin:"0 0 12px",lineHeight:1.05}}>AI Tools for<br/><span style={{color:"#00B4D8"}}>Educators</span></h1>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:15,lineHeight:1.65,margin:0}}>Lesson plans, assessments, parent communications, and curriculum design.</p>
    </section>
    <section style={{maxWidth:720,margin:"0 auto",padding:"28px 20px 0"}}>
      <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.12)",borderRadius:14,padding:"20px 24px"}}>
        <h2 style={{margin:"0 0 12px",fontSize:15,fontWeight:700,color:"#fff"}}>Create Lesson Plan</h2>
        <div style={{display:"flex",gap:8}}><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Grade 7 science, photosynthesis, 45 minutes, project-based..." style={{flex:1,background:"#172D48",border:"1px solid rgba(0,180,216,0.15)",borderRadius:8,padding:"10px 12px",color:"#e2e8f0",fontSize:13,outline:"none",fontFamily:"system-ui"}}/>
